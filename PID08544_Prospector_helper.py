### Python script was updated on 2026/05/18 by Jakob M. Helton.
    ...
    if np.any(np.array(observation_kinds) == 'spectrum'):
        ...
        if contains_nirspec and contains_miri_lrs:
            ...
            model_params['eline_sigma'] = {
                ...
                'depends_on': transform_eline_sigma_across_instruments,
            }
        ...
        if contains_nirspec:
            ...
            if contains_miri_lrs:
                ...
                model_params['eline_sigma_nirspec'] = {
                    'name': 'emission_line_velocity_dispersion_nirspec', 
                    'units': u.km/u.s, 
                    'N': 1, 
                    'isfree': True, 
                    'init': 1e+2, 
        ...
        if contains_miri_lrs:
            ...
            if contains_nirspec:
                ...
                model_params['eline_sigma_miri_lrs'] = {
                    'name': 'emission_line_velocity_dispersion_miri_lrs', 
                    'units': u.km/u.s, 
                    'N': 1, 
                    'isfree': True, 
                    'init': 1e+2, 
...
def transform_zred_to_agebins(zred, tbirth, nbins=6, agebin1=np.log10(3e+6), agebin2=np.log10(1e+7), agebin3=np.log10(3e+7), 
    **dictionary_for_extras):
    ...
    elif agebin1 is None or np.log10(1e+9*tuniv) <= agebin1:

        agelims = np.linspace(0.0, np.log10(1e+9*tuniv), nbins+1).tolist()

    elif agebin2 is None or np.log10(1e+9*tuniv) <= agebin2:

        agelims = [0] + np.linspace(agebin1, np.log10(1e+9*tuniv), nbins-0).tolist()

    elif agebin3 is None or np.log10(1e+9*tuniv) <= agebin3:
...
def build_observations(filename_spec, filename_phot, index_phot, maximumSNR=20.0, polynomial_order=3):
...
def derive_analytic_emission_line_properties(redshift, wavelength_Angstroms, eline_wavelength_Anstroms, 
    ...
    eline_flux_erg_s_cm2 = (3631*jansky_to_cgs)*(eline_flux_normalized*continuum_flux/(1.0 + redshift))
