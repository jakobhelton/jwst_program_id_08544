...
===================================
The following Python script was last updated on 2026/05/19 by Jakob M. Helton.
...
This directory must contain the following :
  - jwst_miri_psf_0002.fits (MIRI/LRS PSF model)
  - resolving_power.csv (tabulated resolution versus wavelength)
...
c_kms = 2.99792458e+5 # speed of light, in units of km/s
...
wave_O2__3727 = 1e-4*rest_wave_O2__3727*(1.0 + redshift) # microns, observed-frame
wave_O2__3729 = 1e-4*rest_wave_O2__3729*(1.0 + redshift) # microns, observed-frame
wave_Ne3_3869 = 1e-4*rest_wave_Ne3_3869*(1.0 + redshift) # microns, observed-frame
wave_Ne3_3968 = 1e-4*rest_wave_Ne3_3968*(1.0 + redshift) # microns, observed-frame
wave_He1_3971 = 1e-4*rest_wave_He1_3971*(1.0 + redshift) # microns, observed-frame
wave_He1_4027 = 1e-4*rest_wave_He1_4027*(1.0 + redshift) # microns, observed-frame
wave_Hd__4103 = 1e-4*rest_wave_Hd__4103*(1.0 + redshift) # microns, observed-frame
wave_Hg__4342 = 1e-4*rest_wave_Hg__4342*(1.0 + redshift) # microns, observed-frame
wave_O3__4363 = 1e-4*rest_wave_O3__4363*(1.0 + redshift) # microns, observed-frame
wave_He1_4471 = 1e-4*rest_wave_He1_4471*(1.0 + redshift) # microns, observed-frame
wave_He2_4686 = 1e-4*rest_wave_He2_4686*(1.0 + redshift) # microns, observed-frame
wave_Hb__4863 = 1e-4*rest_wave_Hb__4863*(1.0 + redshift) # microns, observed-frame
wave_O3__4959 = 1e-4*rest_wave_O3__4959*(1.0 + redshift) # microns, observed-frame
wave_O3__5007 = 1e-4*rest_wave_O3__5007*(1.0 + redshift) # microns, observed-frame
wave_He1_5876 = 1e-4*rest_wave_He1_5876*(1.0 + redshift) # microns, observed-frame
wave_O1__6300 = 1e-4*rest_wave_O1__6300*(1.0 + redshift) # microns, observed-frame
wave_O1__6364 = 1e-4*rest_wave_O1__6364*(1.0 + redshift) # microns, observed-frame
wave_N2__6548 = 1e-4*rest_wave_N2__6548*(1.0 + redshift) # microns, observed-frame
wave_Ha__6565 = 1e-4*rest_wave_Ha__6565*(1.0 + redshift) # microns, observed-frame
wave_N2__6583 = 1e-4*rest_wave_N2__6583*(1.0 + redshift) # microns, observed-frame
wave_S2__6716 = 1e-4*rest_wave_S2__6716*(1.0 + redshift) # microns, observed-frame
wave_S2__6731 = 1e-4*rest_wave_S2__6731*(1.0 + redshift) # microns, observed-frame
...
def inspect_spectra(pathname, filenames_s2d=None, filenames_x1d=None, ellipses=False, redshift=redshift, 
    ...
    if not filenames_s2d and not filenames_x1d:
        ...
        for temp_index, filename_s2d in enumerate(filenames_s2d):
            ...
            # fig.suptitle(fr'$\texttt{{{suptitle}}}$', x=0.515, y=0.9775, fontsize=20)
            ...
            # vmin_zscale, vmax_zscale = np.nanmin(data_masked_sigma_clipped), np.nanmax(data_masked_sigma_clipped), 
            ...
            for index, axis in enumerate(grid):
                ...
                if ellipses and position_nod1 is not None and position_nod2 is not None:
                    ...
                    list_of_line_wavelengths = [
                        wave_O2__3727, wave_O2__3729, 
                        wave_Hb__4863, wave_O3__4959, 
                        wave_O3__5007, wave_Ha__6565, 
                        wave_S2__6716, wave_S2__6731, 
                    ]
                ...
                if index == 0: 
                    ...
                    xlabel = fr'$\mathrm{{Rest-Frame\ Wavelength}}\ \mathrm{{at}}\ z = {redshift:.2f}'
                    xlabel += fr'\ \left[ \mathrm{{microns}} \right]$'

                    ax_top.set_xlabel(xlabel, fontsize=20, labelpad=12)
                    ax_top.set_xlim(xmin/(1.0 + redshift), xmax/(1.0 + redshift))
        ...
        for temp_index, (filename_s2d, filename_x1d) in enumerate(zip(filenames_s2d, filenames_x1d)):
            ...
            # fig.suptitle(fr'$\texttt{{{suptitle.replace('_s2d', '')}}}$', x=0.515, y=0.9875, fontsize=20)
            ...
            for index, axis in enumerate(grid):
                ...
                if index == 0:
                    ...
                    vlines = wavelengths_lines.tolist()

                    ax.vlines(vlines, ymax-10+offset, ymax+offset, colors=colors_5[3], ls='-', lw=3, alpha=1.0, zorder=2)
                    ax.vlines(vlines, ymin-offset, ymin+10+offset, colors=colors_5[3], ls='-', lw=3, alpha=1.0, zorder=2)
                    ...
                    ax.vlines(vlines, 1.1*ymin, 1.1*ymax, colors=colors_5[3], ls=':', lw=3, alpha=1.0, zorder=0)
...
def plot_slit_overlay(directories):
    ...
    """
    Plots the slit overlays from each of the visits on an RGB cutout surrounding JADES-GS-z14-0.
    ...
    # Reads in thumbnails to overlay slit on an RGB image around JADES-GS-z14-0

    F444W = fits.open(f'{directories[0]['Thumbnails']}/183348_size_20.0_arcsec_NRC_F444W.fits')
    F444W_head, F444W_data = F444W[1].header, F444W[1].data
    F444W_wcs = WCS(F444W_head)
    F444W.close()

    F277W = fits.open(f'{directories[0]['Thumbnails']}/183348_size_20.0_arcsec_NRC_F277W.fits')
    F277W_head, F277W_data = F277W[1].header, F277W[1].data
    F277W_wcs = WCS(F277W_head)
    F277W.close()

    F115W = fits.open(f'{directories[0]['Thumbnails']}/183348_size_20.0_arcsec_NRC_F115W.fits')
    ...
    for i, directory in enumerate(directories):
        ...
        # Plots the slit on an RGB image around JADES-GS-z14-0 using the previously read in thumbnails
...
def run_pipeline_full(directories, stage1=True, stage2=True, stage3=True, tweak=True, sigma=3.0, extraction_type='optimal',
    bkg_subtract_list=[True, False], extra_directories_for_spec3=[], mask_trace_width=5, offset=+1.0):
            ...
            if not bkg_subtract:
                ...
                for i in [0, 1]:
                    ...
                    with fits.open(f'{filenames[i]}') as hdul_s2d:
                        ...
                        filenames_split = os.path.basename(filenames_bsub[i]).split('_'); _filename_ = ''
    ...
    if stage3:
        ...
        for i in range(2):
            ...
            ax.vlines(wavelengths_lines, -1.1*amax, +1.1*amax, colors='grey', ls=':', lw=3, alpha=1.0, zorder=0)
...
def model_EmissionLinesAndContinuum_O2(wave, x0, x1, x2, sigma, redshift, flux_O2__Doub, ratio_O2__Doub):
    ...
    obs_wave_O2__3727 = 1e-4*rest_wave_O2__3727*(1.0 + redshift) # microns, observed-frame
    obs_wave_O2__3729 = 1e-4*rest_wave_O2__3729*(1.0 + redshift) # microns, observed-frame

    # Calculates the velocity dispersions after convolving with the line-spread function

    sigma_O2__3727 = np.sqrt(
        np.square((sigma/c_kms)*obs_wave_O2__3727) + 
        np.square(obs_wave_O2__3727/(2.355*get_resolving_power(obs_wave_O2__3727))))
    sigma_O2__3729 = np.sqrt(
        np.square((sigma/c_kms)*obs_wave_O2__3729) + 
...
def model_EmissionLinesAndContinuum_O3(wave, x0, x1, x2, sigma, redshift, flux_Hb__4863, flux_O3__5007):
    ...
    obs_wave_Hb__4863 = 1e-4*rest_wave_Hb__4863*(1.0 + redshift) # microns, observed-frame
    obs_wave_O3__4959 = 1e-4*rest_wave_O3__4959*(1.0 + redshift) # microns, observed-frame
    obs_wave_O3__5007 = 1e-4*rest_wave_O3__5007*(1.0 + redshift) # microns, observed-frame

    # Calculates the velocity dispersions after convolving with the line-spread function

    sigma_Hb__4863 = np.sqrt(
        np.square((sigma/c_kms)*obs_wave_Hb__4863) + 
        np.square(obs_wave_Hb__4863/(2.355*get_resolving_power(obs_wave_Hb__4863))))
    sigma_O3__4959 = np.sqrt(
        np.square((sigma/c_kms)*obs_wave_O3__4959) + 
        np.square(obs_wave_O3__4959/(2.355*get_resolving_power(obs_wave_O3__4959))))
    sigma_O3__5007 = np.sqrt(
        np.square((sigma/c_kms)*obs_wave_O3__5007) + 
...
def model_EmissionLinesAndContinuum_He1(wave, x0, x1, x2, sigma, redshift, flux_He1_5876):
    ...
    obs_wave_He1_5876 = 1e-4*rest_wave_He1_5876*(1.0 + redshift) # microns, observed-frame

    # Calculates the velocity dispersions after convolving with the line-spread function

    sigma_He1_5876 = np.sqrt(
        np.square((sigma/c_kms)*obs_wave_He1_5876) + 
...
def model_EmissionLinesAndContinuum_Ha(wave, x0, x1, x2, sigma, redshift, flux_Ha__6565):
    ...
    obs_wave_Ha__6565 = 1e-4*rest_wave_Ha__6565*(1.0 + redshift) # microns, observed-frame

    # Calculates the velocity dispersions after convolving with the line-spread function

    sigma_Ha__6565 = np.sqrt(
        np.square((sigma/c_kms)*obs_wave_Ha__6565) + 
...
def model_EmissionLinesAndContinuum_Ha_with_S2(wave, x0, x1, x2, sigma, redshift, flux_Ha__6565, flux_S2__Doub, ratio_S2__Doub):
    ...
    obs_wave_Ha__6565 = 1e-4*rest_wave_Ha__6565*(1.0 + redshift) # microns, observed-frame
    obs_wave_S2__6716 = 1e-4*rest_wave_S2__6716*(1.0 + redshift) # microns, observed-frame
    obs_wave_S2__6731 = 1e-4*rest_wave_S2__6731*(1.0 + redshift) # microns, observed-frame
...
def emission_line_fitting(pathname, wave, flux, flux_error, scale_covar=True, include_S2=True, bootstrap=int(1e+2)):
    ...
    sigma_bool, sigma_init, sigma_min, sigma_max = True, 1e+2, 1e+1, 1e+4
    redshift_bool, redshift_init, redshift_min, redshift_max = True, redshift, redshift - 1e-1, redshift + 1e-1
    ...
    for index in range(bootstrap):
        ...
        try:
            ...
            # The velocity dispersion and redshift derived here will be used for other emission lines to provide robust uncertainties
            ...
            Parameters_O3.add_many(
                ...
                ('sigma', sigma_init, sigma_bool, sigma_min, sigma_max),
                ('redshift', redshift_init, redshift_bool, redshift_min, redshift_max),
            ...
            xmin_O3 = 1e-4*np.mean([rest_wave_Hb__4863, rest_wave_O3__5007])*(1.0 + redshift) - wave_delta
            xmax_O3 = 1e-4*np.mean([rest_wave_Hb__4863, rest_wave_O3__5007])*(1.0 + redshift) + wave_delta
            ...
            temp_sigma, temp_redshift = Results_O3.params['sigma'].value, Results_O3.params['redshift'].value
            ...
            Parameters_O2.add_many(
                ...
                ('x1', x1_init, x1_bool, x1_min, x1_max),
                ('x2', x2_init, x2_bool, x2_min, x2_max),
                ('sigma', temp_sigma, False, sigma_min, sigma_max),
                # ('sigma', sigma_init, sigma_bool, sigma_min, sigma_max),
                ('redshift', temp_redshift, False, redshift_min, redshift_max),
                # ('redshift', redshift_init, redshift_bool, redshift_min, redshift_max),
            ...
            if include_S2: 
                ...
                xmin_Ha = 1e-4*np.mean([rest_wave_Ha__6565, 
                    np.mean([rest_wave_S2__6716, rest_wave_S2__6731])])*(1.0 + temp_redshift) - wave_delta
                xmax_Ha = 1e-4*np.mean([rest_wave_Ha__6565, 
                    np.mean([rest_wave_S2__6716, rest_wave_S2__6731])])*(1.0 + temp_redshift) + wave_delta
                ...
                xmin_Ha = 1e-4*rest_wave_Ha__6565*(1.0 + temp_redshift) - wave_delta
                xmax_Ha = 1e-4*rest_wave_Ha__6565*(1.0 + temp_redshift) + wave_delta

            xmin_O2 = 1e-4*np.mean([rest_wave_O2__3727, rest_wave_O2__3729])*(1.0 + temp_redshift) - wave_delta
            xmax_O2 = 1e-4*np.mean([rest_wave_O2__3727, rest_wave_O2__3729])*(1.0 + temp_redshift) + wave_delta

            xmin_He1 = 1e-4*rest_wave_He1_5876*(1.0 + temp_redshift) - wave_delta
            xmax_He1 = 1e-4*rest_wave_He1_5876*(1.0 + temp_redshift) + wave_delta
            ...
            var_names = np.concatenate([['model', 'x0', 'x1', 'redshift', 'sigma'], 
                ...
                for j, column in enumerate(columns[1:]):
                    ...
                    if column in Results.var_names:
                        ...
                        if 'sigma' in column: 
                            ...
                            if 'O2__Doub' in column: temp_wave = np.mean([wave_O2__3727, wave_O2__3729])*u.um
                            elif 'Hb__4863' in column: temp_wave = wave_Hb__4863*u.um
                            elif 'O3__4959' in column: temp_wave = wave_O3__4959*u.um
                            elif 'O3__5007' in column: temp_wave = wave_O3__5007*u.um
                            elif 'He1_5876' in column: temp_wave = wave_He1_5876*u.um
                            elif 'Ha__6565' in column: temp_wave = wave_Ha__6565*u.um
                            elif 'S2__Doub' in column: temp_wave = np.mean([wave_S2__6716, wave_S2__6731])*u.um
    ...
    if bootstrap > 1: 
        ...
        # Calculates redshift percentiles

        redshift_samples = Results_df['redshift'].values
        redshift_samples_val = np.array([sample[0] for sample in redshift_samples if not np.isnan(sample[0])])
        redshift_samples_err = np.array([sample[1] for sample in redshift_samples if not np.isnan(sample[1])])

        redshift_p16, redshift_p50, redshift_p84  = np.percentile(redshift_samples_val, [16, 50, 84])

        redshift_mean, redshift_std = np.nanmean(redshift_samples_val), np.nanstd(redshift_samples_val)

        Results_bootstrapped_statistics.append([
            redshift_p16, redshift_p50, redshift_p84, redshift_mean, redshift_std])

        # Calculates sigma percentiles

        sigma_samples = Results_df['sigma'].values
        sigma_samples_val = np.array([sample[0].value for sample in sigma_samples if not np.isnan(sample[0])])
        sigma_samples_err = np.array([sample[1].value for sample in sigma_samples if not np.isnan(sample[1])])

        sigma_p16, sigma_p50, sigma_p84  = np.percentile(sigma_samples_val, [16, 50, 84])

        sigma_mean, sigma_std = np.nanmean(sigma_samples_val), np.nanstd(sigma_samples_val)

        Results_bootstrapped_statistics.append([
            sigma_p16, sigma_p50, sigma_p84, sigma_mean, sigma_std])
